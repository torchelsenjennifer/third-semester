...
# sendo preenchido com " " (espaços), que indica posição disponivel
def cria_jogo():
	...
	for l in range(NUM_LINHAS):
		for c in range(NUM_COLUNAS-3):
			if jogo[l][c] == simbolo and jogo[l+1][c+1] == simbolo and jogo[l+2][c+2] == simbolo and jogo[l][c+3] == simbolo :
				return True
	#quatro posiçoes consecutivas na vertical(de cima para baixo, esquerda para direita)
	for l in range(NUM_LINHAS):
		for c in range(NUM_COLUNAS-3):
...
# contatdor é usado para definir o jogador "x" ou "o" e
...
while True:
	...
	if linha == -1:
		print("Não há linhas disponivel nesta coluna.Jogue novamente")
	else:
	...
	if vencedor(jogador):
		print()
		print("*"*40)
		print("Parabéns Jogadores '{jogador}'. Voce é o vencedor!")
		print("*"*40)
		break
